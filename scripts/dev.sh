#!/usr/bin/env bash
# Dev helper for this mod. One command does each repeated loop.
#
...
#                                 the zip in dist/ (see RELEASING.md)
...
# The mod name comes from the project file, so a rename of the project renames everything here.
MOD="$(basename "$(ls "$REPO"/*.csproj | head -1)" .csproj)"
...
do_verify()  {
  ...
  # asset load from it fails, and combat starts with no background. The result is easy to
  ...
  if pgrep -f "SlayTheSpire2" >/dev/null 2>&1; then
    ...
    echo "  restart the game before you test"
  fi
}

# ── release plumbing ────────────────────────────────────────────────────────
CHANGELOG="$REPO/CHANGELOG.md"
MANIFEST="$REPO/$MOD.json"
DIST="$REPO/dist"
...
do_release() {  # <patch|minor|major|X.Y.Z>
  ...
  # Package the zip that a user installs: one top-level mod folder with the three
  # runtime files that the game loads. It has no pdb, which is for debug only.
  step "package dist/$MOD-v$new.zip"
  local stage="$DIST/stage" src="$GAME_MODS/$MOD"
  rm -rf "$stage"; mkdir -p "$stage/$MOD"
  local f; for f in "$MOD.dll" "$MOD.json" "$MOD.pck"; do
    [ -f "$src/$f" ] || { bad "$src/$f must exist after the publish; the release stops here"; exit 1; }
    cp -f "$src/$f" "$stage/$MOD/"
  done
  local zipfile="$DIST/$MOD-v$new.zip"
  rm -f "$zipfile"
  (cd "$stage" && zip -r -q "$zipfile" "$MOD")
  rm -rf "$stage"
  ls -lh "$zipfile"

  # Get the notes of this version for the GitHub Release body or the Workshop comment. The
  # version heading is dropped, because both paste targets supply their own title. The
  # changelog wraps its bullets to keep the markdown source readable. Both targets wrap text
  # themselves, so the second stage puts each bullet back on one line.
  ...
  # workshop.json takes the new version and the notes as its changeNote. Thus a
  # `ModUploader upload -w workshop` after the release pushes exactly what was packaged.
  if [ -f "$REPO/workshop/workshop.json" ]; then
    step "workshop workspace"
    mkdir -p "$REPO/workshop/content"
    for f in "$MOD.dll" "$MOD.json" "$MOD.pck"; do
...
require_gh() {
  command -v gh >/dev/null 2>&1 || { bad "gh (the GitHub CLI) is not installed; see https://cli.github.com"; exit 1; }
...
do_publish_release() {  # [--force] [--draft] [vX.Y.Z]
  ...
  # The artifacts come first, before this command writes any history. A `release` that never ran
  # would otherwise leave a commit behind for you to undo.
  local zip="$DIST/$MOD-$tag.zip" notes="$DIST/RELEASE_NOTES-$tag.txt"
  ...
  if [ -n "$(git -C "$REPO" status --porcelain)" ]; then
    local extra; extra="$(git -C "$REPO" status --porcelain | cut -c4- \
      | grep -v -E "^($MOD\.json|CHANGELOG\.md|workshop/workshop\.json)$" || true)"
    [ -z "$extra" ] || {
      bad "the working tree has changes other than the release edit: $(echo "$extra" | tr '\n' ' ')"; exit 1; }
    step "commit release: $tag"
    git -C "$REPO" add "$MOD.json" CHANGELOG.md
...
do_doctor() {
  ...
  if [ -d "$GAME_MODS/$MOD" ];       then ok "$MOD mod installed"; else bad "$MOD mod not installed; run scripts/dev.sh publish"; fail=1; fi
  # gh is necessary only for publish-release, so a missing gh is a note, not a failure.
  if ! command -v gh >/dev/null 2>&1; then bad "note: gh is not installed; scripts/dev.sh publish-release needs it (https://cli.github.com)"
...
case "${1:-help}" in
  ...
  env)
    ...
    ;;
  *)
    # The header block only. Stopping at the first line of code keeps the section comments
    # further down out of the help text.
