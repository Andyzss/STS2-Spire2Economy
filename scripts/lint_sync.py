...
MOD = next(REPO.glob("*.csproj")).stem
CODE = REPO / f"{MOD}Code"
IMG = REPO / MOD / "images"
CSV = REPO / "cards.csv"
LOC = REPO / MOD / "localization" / "eng" / "cards.json"
PREFIX = MOD.upper()

# csv display name -> class name, for a card whose class name is not the display name
# with the spaces removed (for example a basic Strike with a mod suffix)
SPECIAL_CLASS: dict[str, str] = {}

# Every entity gets its art from its own class name (see <Mod>Code/Extensions/
...
def parse_builders(text: str) -> list[tuple[int, int]]:
    ...
    from a dynamic var or from a calculation at run time. The csv shows these as "(+ N)",
    not as a literal "0 (N)" pair, so ignore them.
    """
